...
@WebServlet({ "/User", "/User/edit/*", "/User/create", "/User/update", "/User/delete", "/User/reset", "/User/search" })
...
//				String id = request.getPathInfo() != null ? request.getPathInfo().substring(1) : null;
				...
				if (form.getId() == null || form.getId().trim().isEmpty()) {
					request.setAttribute("message", "Vui lòng chọn người dùng để cập nhật!");
						...
						|| form.getEmail().trim().isEmpty()) {
					request.setAttribute("message", "Vui lòng chọn người để cập nhật!");
				} else {
					dao.update(form);
				...
				if (keyword == null || keyword.trim().isEmpty()) {
					// Nếu người dùng không nhập gì mà nhấn tìm
					list = dao.findAll(); // trả về tất cả người dùng
					request.setAttribute("message","Vui lòng nhập người dùng tìm kiếm.");
				} else {
					list = dao.findByKeyword(keyword.trim());
					if (list.isEmpty()) {
						request.setAttribute("message", "Không tìm thấy kết quả nào cho '" + keyword + "'!");
					} else {
						request.setAttribute("message", "Tìm thấy " + list.size() + " kết quả cho '" + keyword + "'.");
					}
				}
			}
